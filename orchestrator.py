...
def _call_agent_once(cfg, app, phase, rnd, agent, prompt):
    ...
    # Stagger same-provider calls (runtime.provider_min_gap_seconds) so burst
    # fan-outs don't trip per-minute rate limits.
...
def _phase_contract(key):
    ...
    if key == "task_assignments":
        return _TASKS_JSON_INSTRUCTION + _FLOWS_JSON_INSTRUCTION
    if key == "app_features":
        return _REQUIREMENTS_JSON_INSTRUCTION
    if key == "tech_specs":
...
def process_phase(cfg, app, app_dir, phasedef, original_prompt, prior_outputs,
    ...
    # Rolling summaries (token diet): a local model compresses this phase's
    # discussion once; later phases read the summary instead of replaying the
    # raw transcript (the full text stays on disk). Best-effort.
    if not is_build and bool(cget(cfg, "runtime.rolling_summaries_enabled", True)):
        try:
            _tag = (cfg.get("_resolved", {}) or {}).get("ollama_model") \
                or str(cget(cfg, "models.ollama", "") or "")
            if _tag and _tag in set(lmlib.installed_models_cached()) \
                    and len(transcript) > 3000:
                _ask = ("Summarize this multi-agent product discussion in at "
                        "most 350 words. PRESERVE: final decisions, real "
                        "disagreements and who held them, rejected options "
                        "with the reason, exact names/numbers/contracts. "
                        "Drop pleasantries and restated agreement.\n\n"
                        + transcript[-16000:])
                _out, _e, _code, _c = resolve_runner("local:%s" % _tag)(
                    cfg, _ask, 120)
                if _code == 0 and (_out or "").strip():
                    state.setdefault("phase_summaries", {})[key] = \
                        _out.strip()[:4000]
        except Exception:  # noqa: BLE001 - summaries are an optimization
            pass
...
def _run_app_pipeline(cfg, app, app_dir, prompt):
    ...
    try:
        ...
        while i < len(phases):
            ...
            _sums = state.get("phase_summaries") \
                if bool(cget(cfg, "runtime.rolling_summaries_enabled", True)) else None
