...
PARENTS=(multi-app-exp7)
MAX_BUILDS=3; MAX_PARENTS=2
...
while true; do
  ...
  # Children next — builds are the bottleneck. exp4's apps jump the queue
  # (they were requested before the 51-app batch).
  for d in $(queue_order_dirs) "$ROOT"/waylay/ "$ROOT"/practice-loop/ "$ROOT"/proof/ "$ROOT"/returnwise/ "$ROOT"/vervecoach/ "$ROOT"/*/; do
    [ "$builds_running" -ge "$MAX_BUILDS" ] && break
    a=$(basename "$d")
