...
|---------|---------------------|
| `cd orchestrator-v2-source && python3 -m unittest discover -s tests` | **PASS — covered by the strict run below** |
| `cd orchestrator-v2-source && python3 -W error::ResourceWarning -m unittest discover -s tests` | **PASS — `Ran 191 tests / OK`** (clean with ResourceWarning promoted to error — no leaked file handles) |
| `cd orchestrator-v2-source/gui && swift build -c release` | **PASS — `Build complete!`** (zero warnings/errors) |
| `cd orchestrator-v2-source/gui && swift test` | **PASS — `Executed 37 tests, with 0 failures`** (OrchestratorGUITests / EngineLogicTests) |
| `cd orchestrator-v2-source && python3 orchestrator.py --doctor` | PASS — root resolves to `/Users/pchordia/Documents/iOS-App-Factory` (exists=True); CLIs found: codex, claude, gemini, agy, ollama, git, python3; 9 workflows listed; no leaked API-key env vars |
| `cd orchestrator-v2-source && python3 orchestrator.py --doctor --json` | PASS — valid JSON; `build_capable: true`; includes the `local_models` block (server_running, selected, registry with installed flags) |
