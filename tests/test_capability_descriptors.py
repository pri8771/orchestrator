...
class TestRegistryCompleteness(unittest.TestCase):
    ...
    def test_no_id_claims_streams_or_token_usage_yet(self):
        # 6.2's api: runners flip these first; until then a True here would be
        # a decorative control (§16). The api: PREFIX rule is the one allowed
        # forward declaration — it has no resolvable ids until 6.2 lands.
        for aid, caps in orch.AGENT_CAPABILITIES.items():
